...
impl<T> Node<T> {
    ...
    pub unsafe fn finish_init(node: Pin<&Self>) {
        // Now convert it into a pointer. Note the use of get_ref rather than
        // Deref; this ensures that the intermediate reference has the lifetime
        // of the reference within the pin, rather than the lifetime of our node
        // local.
        let nnn = NonNull::from(node.get_ref());
        node.next.set(nnn);
        node.prev.set(nnn);
    }
    ...
    pub fn detach(self: Pin<&Self>) {
        ...
        // Link to ourselves.
        self.prev.set(NonNull::from(self.get_ref()));
        self.next.set(NonNull::from(self.get_ref()));
    }
...
impl<T> List<T> {
    ...
    pub unsafe fn finish_init(list: Pin<&Self>) {
        // Safety: this is safe if our own safety contract is upheld.
        unsafe {
            Node::finish_init(list.root());
        }
    }

    fn root(self: Pin<&Self>) -> Pin<&Node<T>> {
        unsafe { Pin::new_unchecked(&self.get_ref().root) }
    }
...
macro_rules! create_list {
    ($var:ident, $t:ty) => {
        ...
        #[allow(unused_unsafe)]
        let $var = unsafe {
            core::mem::ManuallyDrop::into_inner($crate::List::<$t>::new())
        };
        ...
        #[allow(unused_unsafe)]
        unsafe {
            $crate::List::finish_init($var.as_ref());
        }
    };
    ($var:ident) => {
        // Safety: we discharge the obligations of `new` by pinning and
        ...
        #[allow(unused_unsafe)]
        let $var = unsafe {
            core::mem::ManuallyDrop::into_inner($crate::List::new())
        };
        pin_utils::pin_mut!($var);
        // Safety: the value has not been operated on since `new` except for
        // being pinned, so this operation causes it to become valid and safe.
...
macro_rules! create_node {
    ($var:ident, $dl:expr, $w: expr) => {
        ...
        // finishing the value, below, before it can be dropped.
        let $var = unsafe {
            core::mem::ManuallyDrop::into_inner($crate::Node::new(
                $dl, $w,
            ))
        };
        pin_utils::pin_mut!($var);
        ...
        // being pinned, so this operation causes it to become valid and safe.
        unsafe {
            $crate::Node::finish_init($var.as_mut());
        }
    };
}
...
mod tests {
    ...
    fn create_and_drop_node() {
        create_node!(list, (), exploding_waker());
    }
    */
}
